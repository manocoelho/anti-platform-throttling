...
1. **O ritmo de entrada é diferente do ritmo de saída.** O administrador cria uma
   campanha de 10.000 envios num único POST. As plataformas aceitam poucas
   requisições por segundo (3 a 10, dependendo da plataforma). Processar de
   forma síncrona significaria uma requisição HTTP de dezenas de minutos.
