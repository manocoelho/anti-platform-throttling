...
- **Bucket cheio libera rajada após inatividade.** Um bucket parado enche até a
  ...
  requisições. Mitigado por manter `burst_capacity <= estimated_limit_rps` da
  plataforma — invariante verificada em
  `tests/unit/test_domain.py::test_burst_nao_passa_do_limite_estimado`.
