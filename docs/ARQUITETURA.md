...
graph LR
    ...
    r1["apt.retry.youtube.1<br/>TTL 1s"]
    r2["apt.retry.youtube.2<br/>TTL 5s"]
    r3["apt.retry.youtube.3<br/>TTL 30s"]
    ...
    w1 -.->|"rk: tier.N.youtube"| retryex
    retryex --> r1
    retryex --> r2
    retryex --> r3

    r1 -.->|"TTL expira → DLX = apt.tasks<br/><b>rk declarada: youtube</b>"| tasks
    r2 -.-> tasks
    r3 -.-> tasks
...
> Por brevidade, o diagrama mostra só as três filas de retry do YouTube
> (`apt.retry.youtube.1/2/3`). O Instagram tem o conjunto equivalente
> (`apt.retry.instagram.1/2/3`), com o mesmo desenho — devolvendo TTL expirado para
> `apt.tasks.instagram` via `x-dead-letter-routing-key: instagram`.
...
**As filas de retry declaram `x-dead-letter-routing-key` explicitamente, uma por
plataforma** — não foi sempre assim, e a história é a lição. A primeira versão tinha 3
filas (uma por degrau, compartilhadas entre plataformas) e omitia essa chave, na
suposição de que o RabbitMQ preservaria a routing key ORIGINAL da mensagem (a
plataforma) ao expirar o TTL. Era um bug real: a routing key preservada é a que a
mensagem tinha ao *entrar* na fila de retry — `tier.N.<plataforma>`, usada só para
escolher a fila de espera — não a plataforma sozinha. Sem o binding certo, cada retry
chegava inroteável e era descartado em silêncio: toda tarefa adiada era perdida ao
expirar o TTL. A correção segrega as filas por plataforma×degrau (6 no total) e declara
a routing key de destino. Detalhe completo, causa raiz e evidência de reprodução em
...
sequenceDiagram
    ...
    W->>MQ: publish apt.retry.youtube.1 (defers+1, attempt intacto)
