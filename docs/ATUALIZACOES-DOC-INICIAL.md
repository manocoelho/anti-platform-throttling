...
| Testes | "Pytest" | **117 unitários + 4 de integração + 3 cenários de carga** |
...
plataforma em 4×. **O bug apareceria exatamente ao escalar** — sob carga, quando o limite
...
teste provaria apenas que `16 < 20` — aritmética, não engenharia.
...
que envia exatamente 16 requisições no primeiro milissegundo de cada segundo respeita
16 req/s e exibe padrão obviamente automatizado: intervalos idênticos, variância zero. Os
...
Três bugs reais que valem contar na apresentação, porque são específicos deste domínio.
...
Os cenários de carga, resiliência e escala **não foram executados** na máquina de
desenvolvimento, por uma falha do runtime de containers do Docker Desktop (diagnóstico
completo em [RESULTADOS-TESTES.md, Parte 2](RESULTADOS-TESTES.md#parte-2--pendente-de-execução)).

Os testes estão implementados e imprimem o relatório em markdown já formatado. Falta
executá-los numa máquina com Docker funcional e colar os números.
