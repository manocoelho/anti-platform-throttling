...
**conteúdo** (4 req/s por URL) e depois o da **plataforma** (16 req/s). O eixo do
...
sistemas de detecção procuram. Limitar apenas a plataforma permitiria 16 req/s numa
URL só: dentro do limite agregado, e obviamente artificial.

### Como demonstrar

```bash
# Demanda de 30 req/s contra um limite de 16 req/s
curl -X POST localhost:8000/campaigns -H 'Content-Type: application/json' -d '{
  "name":"Demo rate limit","platform":"youtube","total_sends":400,
  "target_rate_per_min":1800,"jitter_strategy":"humanized",
  "contents":[{"url":"https://yt/a"},{"url":"https://yt/b"},{"url":"https://yt/c"}]}'

sleep 20
curl -s localhost:9001/admin/stats | python -m json.tool
#  ↑ peak_rps deve ficar <= 20 (o limite da plataforma) e total_throttled = 0
...
POST; as plataformas aceitam 20 req/s. Sem fila, a requisição HTTP levaria 8 minutos.
Com fila, a API responde em milissegundos e a fila absorve o pico.
...
# A DLQ com contexto consultável por SQL
curl -s localhost:8000/admin/failures | python -m json.tool
```
