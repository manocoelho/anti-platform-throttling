...
O sistema resolve isso com um **token bucket cujo estado vive no Redis**, consultado por um **script Lua atômico**. O teste `tests/load/scale_test.py` mede exatamente essa propriedade: 1, 3, 5 workers com o pico observado pela plataforma constante.
...
3. **Falta idempotência ponta a ponta.** A semântica *at least once* do RabbitMQ permite envio duplicado se um worker morrer entre enviar e dar ack. É a limitação mais séria do projeto ainda em aberto.
